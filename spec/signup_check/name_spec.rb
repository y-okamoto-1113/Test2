...
describe "validation name" do #バリデーションかけるときに、「uniqueness: true」にしてるから、データベース消してからチェックしなアカン
...
######################################################################### 
  describe "number only" do #long,short
    example "long" do
      user.name = '1234567890987654098765412345678'
      # binding.pry                     ####### ここで止めれる
...
#########################################################################
  describe "Alphabet only" do #long,short,big,small
    example "BIG long" do
      user.name ='AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'
      expect(user.save).to eq true
    end
    ...
    example "small long" do
      user.name = 'aaaaaaaaaaaaaaaaaaaaaaaaaaaa'
      expect(user.save).to eq true
    end
...
############################################################################
  describe "multibyte-character only" do #漢字、ひらがな、カタカナ、long,short
    example "漢字only long" do
      user.name = '岡本太郎岡本太郎岡本太郎岡本太郎岡本太郎岡本太郎岡本太郎岡本太郎岡本太郎'
      expect(user.save).to eq true
    end
    ...
    example "カタカナonly long" do
      user.name = 'アアアアアアアアアアアアアアアアアアアアアアアアアアアアアアア'
      expect(user.save).to eq true
    end
